...
proceduralControl.addBlockOrMesh(
    "wedge",
    top=_BLOCK_ID_3_WEDGE_FACE_UVS,
    bottom=_BLOCK_ID_3_WEDGE_FACE_UVS,
    left=_BLOCK_ID_3_WEDGE_FACE_UVS,
    right=_BLOCK_ID_3_WEDGE_FACE_UVS,
    back=_BLOCK_ID_3_WEDGE_FACE_UVS,
    front=_BLOCK_ID_3_WEDGE_FACE_UVS,
    direction="east",
)
...
proceduralControl.addBlockOrMesh(
    "obj",
    meshFilePath=os.path.join(_MODULE_DIR, "sphereExample.obj"),
)
...
def _chunk_geometry_payload(worker_chunk, max_faces_per_part):
    ...
    if index_rows % 3 != 0:
        raise ValueError(f"GeomTriangles index count must be divisible by 3, got {index_rows}")

    vertex_stride = len(vertex_data) // vertex_rows
    if vertex_stride * vertex_rows != len(vertex_data):
        raise ValueError("Vertex data byte count does not align with vertex rows")

    primitive_vertices = worker_chunk.tri.get_vertices()
    if primitive_vertices is None:
        indices = np.array(
            [worker_chunk.tri.get_vertex(index) for index in range(index_rows)],
            dtype=index_dtype,
        )
    else:
        index_data = primitive_vertices.get_handle().get_data()
        indices = np.frombuffer(index_data, dtype=index_dtype, count=index_rows).copy()

    highest_index = int(indices.max()) if indices.size else -1
    if highest_index >= vertex_rows:
        raise ValueError(
            f"Generated geometry references vertex {highest_index}, but only has {vertex_rows} vertex rows"
        )

    index_data = indices.tobytes()

    triangle_count = index_rows // 3
    max_triangles_per_part = max(1, int(max_faces_per_part) * 2)
    ...
    vertex_bytes = np.frombuffer(vertex_data, dtype=np.uint8).reshape((vertex_rows, vertex_stride))
    parts = []

    for triangle_start in range(0, triangle_count, max_triangles_per_part):
        part_triangle_count = min(max_triangles_per_part, triangle_count - triangle_start)
        index_start = triangle_start * 3
        index_count = part_triangle_count * 3
        part_indices = indices[index_start:index_start + index_count]
        unique_vertex_indices, local_indices = np.unique(part_indices, return_inverse=True)

        parts.append({
            "vertex_rows": len(unique_vertex_indices),
            "vertex_data": vertex_bytes[unique_vertex_indices].copy().tobytes(),
            "index_rows": index_count,
            "index_data": local_indices.astype(index_dtype).tobytes(),
            "index_type": index_type,
        })
